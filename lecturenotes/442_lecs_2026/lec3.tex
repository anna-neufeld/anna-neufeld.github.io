\section{Thursday, Feb 12: adding more predictor variables!}

Today, we will finish our bias-variance tradeoff derivation and discuss its implications.

Last time, we talked about the pros and cons of linear regression vs. KNN, but all of our visualizations were of regression with a single predictor variable $x \in \mathbb{R}$. Today, we will consider regression in high dimensions, where  $x \in \mathbb{R}^p$.
...
\end{equation}
where $\mathcal{F}$ is some class of functions. On your homework, you will argue that if $\mathcal{F}$ were totally unconstrained, we would want to set $\hat{f}(x) = \mathrm{E}[Y \mid X=x]$. 
